...
#####################################################################################
###################### CHIMERA SCREEN (CONTINUED) ###################################
#####################################################################################

# Import master tables
df <- read.table("df.txt", sep = "\t", header = T)

# import chimeric sequences
chimeras.fasta <- readDNAStringSet("chimeras.fas", format = "fasta")
chimera.list <- names(chimeras.fasta)
chimera.list <- sub(";ab=.*", "", chimera.list)

# Remove chimeric ASVs from master ASV table
df <- df[!(df$Run_Wide_ASV_Name %in% chimera.list),]
...
param_file <- list.files(pattern = "parameters*", path = "../")
input.metadata <- read.xlsx(xlsxFile = paste0("../", param_file), sheet = "Sample Metadata")
...
names(df) <- c("Sample", "Replicate", "Sample_ASV_Name", "Reads", "Sample_ASV_Sequence", 
               "Run_ASV_Name", "Run_ASV_Sequence", "Tax")

# Add sequence length and ambiguous base count to tables
df$Sample_ASV_Length <- nchar(df$Sample_ASV_Sequence)
df$Sample_ASV_Ns <- nchar(df$Sample_ASV_Sequence) - nchar(gsub("N", "", df$Sample_ASV_Sequence))
df$Sample_ASV_Ns <- as.numeric(df$Sample_ASV_Ns)

# Rename run-wide ASV names to more human-readable form
runwide.names <- df$Run_ASV_Name[order(-df$Reads)]
runwide.names <- runwide.names[!duplicated(runwide.names)]
num_digits <- nchar(length(runwide.names))
temp.df <- data.frame(
  "OldName" = runwide.names,
  "NewName" = sprintf(paste0("ASV_%0", num_digits, "d"), 1:length(runwide.names))
)
df <- merge(df, temp.df, by.x = "Run_ASV_Name", by.y = "OldName", all.x = TRUE)
df <- df[,c("Sample", "Replicate", "NewName", "Reads", "Sample_ASV_Name", "Sample_ASV_Length", "Sample_ASV_Ns", "Sample_ASV_Sequence",
            "Run_ASV_Sequence", "Tax")]
names(df)[c(3,9)] <- c("ASV_ID", "Run_ASV_Consensus_Sequence")
...
#####################################################################################
df <- df[order(df$Sample, df$ASV_ID, -df$Reads),]
...
df.summary <- df %>%
  ...
  summarize(
    Reads = sum(Reads),
    ASV_Consensus_Sequence = first(Run_ASV_Consensus_Sequence),
    Seq_Length = nchar(first(Run_ASV_Consensus_Sequence)),
    Number_N = nchar(first(Run_ASV_Consensus_Sequence)) - nchar(gsub("N", "", first(Run_ASV_Consensus_Sequence))),
...
#####################################################################################
################## MERGE ASV COMPONENT READS BY RUN-WIDE ASV ########################
#####################################################################################
if(componentreads == 1){
  # Strip ";size=" from ASV name column to match component read file names
  df$Sample_ASV_Name <- sub(";size=.*$", "", df$Sample_ASV_Name)
  
  # Create import/export directory
  fasta_dir <- sprintf("./%s_%s_%s_ASV_Component_Reads", runid, marker, amp.size)
  if (!dir.exists(fasta_dir)) stop("FASTA directory does not exist!")
  
  # Group by Sample and ASV_ID
  df %>%
    group_by(Sample, ASV_ID) %>%
    group_walk(~ {
      sample <- .y$Sample
      ...
      out_file <- file.path(fasta_dir, sprintf("%s|%s|reads-%d_ComponentReads.fasta", sample, asv, sum_reads))
      
      # Construct input FASTA filenames
      fasta_files <- file.path(fasta_dir, paste0(.x$Sample_ASV_Name, "_ComponentReads.fasta"))
      fasta_files <- fasta_files[file.exists(fasta_files)]
      if (length(fasta_files) == 0) {
        warning(sprintf("No FASTA files found for %s|%s", sample, asv))
        return()
      }
...
# Remove Sample_ASV_Name column from df
df <- df[, -5]
...
setColWidths(wb,"By Sample",cols = 2,widths = "13") #asv 
...
if(marker == "COI-5P" & amp.size >= 300){
  output.fasta <- DNAStringSet(df.summary$ASV_Consensus_Sequence)
  names(output.fasta) <- df.summary$ASV_ID
